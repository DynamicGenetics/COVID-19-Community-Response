...
DATA_MAP = {
    'twitter': Dataset(name='Twitter Dataset', data_format='csv', filename='tweets_dataset.csv'),
    'demographics': Dataset(name='Demographics', data_format='csv', filename='demographics.csv'),
    'la_boundaries': Dataset(name='Local Authorities Boundaries', data_format='geojson',
                             filename='boundaries_LAs.geojson'),
    'la_keys': Dataset(name='Local Authorities Keys', data_format='geojson',
                       filename='la_keys.geojson'),
    'covid_cases': Dataset(name='Covid-19 Cases', data_format='geojson',
                           filename='cases.geojson')
}
...
def generate_la_keys(data_filename: str = 'la_keys.geojson'):
    ...
    la_key_df = pd.merge(la_geo.data, la_demog.data[["id_area", "pop", "lhb"]],
                            left_on="lad18cd", right_on="id_area", how="left")
