...
# Get the hourly average of the composite vader tweets.
covidcomphouravg = corona_tweets['vader_comp'].resample('H').mean().to_frame().reset_index().rename(
    columns={"vader_comp": "covid_vader_comp"})
covidcompdayavg = corona_tweets['vader_comp'].resample('D').mean().to_frame().reset_index().rename(
    columns={"vader_comp": "covid_vader_comp"})
