...
# Get the hourly average of the composite vader tweets.
covidcomphouravg = corona_tweets['vader_comp'].resample('H').mean().to_frame().reset_index().rename(columns={"vader_comp": "covid_vader_comp"})
covidcompdayavg = corona_tweets['vader_comp'].resample('D').mean().to_frame().reset_index().rename(columns={"vader_comp": "covid_vader_comp"})
...
plot_hourly_avg = pd.melt(hourly_avg, id_vars=['created_at'], value_vars=['covid_vader_comp', 'vader_comp'])

p = ggplot(plot_hourly_avg) + geom_line(aes(x='created_at', y='value', color='variable'))
p = p + labs(x="Date-Time", y="VADER Compound Sentiment", title="Hourly compound sentiment for all vs COVID tweets")
p = p + theme(axis_text_x = element_text(angle=90, vjust=1))

# Reshape the daily dataframe to long format for plotting
plot_daily_avg = pd.melt(daily_avg, id_vars=['created_at'], value_vars=['covid_vader_comp', 'vader_comp'])

p2 = ggplot(plot_daily_avg) + geom_line(aes(x='created_at', y='value', color='variable'))
p2 = p2 + labs(x="Date-Time", y="VADER Compound Sentiment", title="Daily compound sentiment for all vs COVID tweets")
