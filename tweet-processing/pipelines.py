...
def _bbox_tuple(data: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    data['bbox'] = data['place.bounding_box.coordinates'].apply(lambda c: tuple(map(tuple, c[0][:-1])))
    return data
...
# %%
def match_local_authorities(bbox: BoundingBox, la_df: pd.DataFrame, return_all: bool = False)->MatchingLA:
    ...
    laoi["pop_weight"] = (
        laoi["geometry"].apply(lambda g: (g.intersection(bbox).area / g.area)) * laoi["pop"]
    )
...
def match_reference_la(data):
    ...
    la_map_geo_coords = {c: l for c, l in zip(set_geo_coords, map(match_la, list_geo_coords))}

    data['la'] = data['geo_coords'].apply(lambda c: None if c is None or \
                                          la_map_geo_coords[c] is None else la_map_geo_coords[c])

    set_bbox_coords = set(data[data['la'].isna()]['bbox'].values)
    la_map_bbox_coords = {c: l for c, l in zip(set_bbox_coords, map(match_la, set_bbox_coords))}
    residual_matchings = data[data['la'].isna()]['bbox'].apply(lambda c: la_map_bbox_coords[c])
