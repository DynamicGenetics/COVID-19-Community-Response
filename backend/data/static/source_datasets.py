...
# ++++++++++++++++++++++++++++++++++
SOURCE_WELSH_LSOA = pd.read_csv("source/lsoa_welsh_language_2011.csv", usecols=[2, 3])

SOURCE_WELSH_LA = pd.read_csv(
    "source/la_welsh_frequency_2018-19.csv", usecols=[1, 2, 3, 4]
)


# Read Population data (includes age based data)
SOURCE_POPULATION_LSOA = pd.read_excel(
    "source/lsoa_population_2018-19.xlsx",
...
SOURCE_POPULATION_LA = pd.read_csv("source/la_population_age_2019.csv", usecols=[3, 15])
SOURCE_OVER_65_LA = pd.read_csv("source/la_population_age_2019.csv", usecols=[3, 14])


# Read in IMD data
SOURCE_IMD_LSOA = pd.read_csv("source/lsoa_IMD_2019.csv")

SOURCE_IMD_LA = pd.read_csv("source/la_WIMD_2019.csv")

# Read in population density data
SOURCE_POPDENSITY_LSOA = pd.read_excel(
    "source/lsoa_pop_density_2018-19.xlsx", sheet_name=3, usecols="A,B,E", skiprows=4
)

SOURCE_POPDENSITY_LA = pd.read_csv("source/la_pop_density_2018.csv", usecols=[1, 11])

# Read in Vulnerable and Community Cohesion Data
vulnerable_and_cohesion = pd.read_excel(
    "source/la_vulnerableProxy_and_cohesion.xlsx",
...
SOURCE_INTERNET_ACCESS_LA = pd.read_excel(
    "source/National Survey results - internet use and freqency of access by local authority.xlsx",
    usecols="A,B",
    skiprows=4,  # Data starts on row 5
...
SOURCE_ETHNICITY_LA = pd.read_excel(
    "source/la_lhb_ethnicity.xlsx", sheet_name="By Local Authority", usecols="B:X"
).T
