...
def cleanData(input_path, output_path):
    ...
    cleaned = cleaned.drop(['Specimen date', 'Cases (new)', 'Cumulative cases', 'Testing episodes (new)', 'Cumulative testing episodes'], axis=1) 
    ...
    cleaned['areaID']=cleaned.apply (lambda row: areaCode(row['Local Authority']), axis=1)
    
    # Rename columns to make consistent with other data files used for mapping
    cleaned = cleaned.rename(columns={'Local Authority' : 'la_name', 'Cumulative incidence per 100,000 population' : 'covidIncidence_100k', 'areaID' : 'areaID'})
...
print("Message (phwScraper): Scraped covid data (latest data: {}), columns: {}".format(covid[0],covid[1]))