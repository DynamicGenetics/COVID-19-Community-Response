...
def schedule_runner(run_frequency_in_days, thing_you_want_to_schedule):
    ...
    schedule.every(run_frequency_in_days).days.at("06:00").do(
        thing_you_want_to_schedule
    )
...
def main_scheduler(run_frequency_in_days):
    ...
    if __name__ == "__main__":
        ...
        def main():
            print("hello")
            # run the process immediately on calling the script.
...
def mongoexport_scheduler(run_frequency_in_days):
    ...
    # schedule.every(run_frequency_in_days).days.at("06:00").do(
    #     mongo_ops.export_csv(mongoexport_executable_path, csv_filename)
    # )
